...
//повертає кількість днів у місці у поточній даті
 const int Date:: numberOfDaysInMonth(const int month)const
{
	int numberOfDays= 0;
	 switch(month)
   {
...
//кількість високосних років до поточного
const int Date::howManyLeaps(const int y) const
{
	int sum(0);
	for(int i = 0;i<y;++i)
		if(leapYear(i)) ++sum;
...
//кількість днів у цьому році переведена з повних місяців
const int Date:: howManyDaysToMounth(const int m)const
{
	int sum(0);
		for(int i = 1;i<m;++i){
			sum+=numberOfDaysInMonth(i);
		}
		return sum;
}
//переводимо дату в дні
const int Date:: howManyDaysInDate()const
{
	int sum(0);
	sum=year()*365+howManyLeaps(year())+howManyDaysToMounth(month())+day();//сума усіх днів з 1.1.0
	return sum;
}
//переводить дату в секунди,
time_t Date:: toSeconds() const
{
	//-1 бо поточна дата не враховується
	  return (howManyDaysInDate()-1)*24*60*60;
}
